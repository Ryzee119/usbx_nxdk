...
static ULONG   simulated_address_msw =  0x0011;
static ULONG   simulated_address_lsw =  0x22334456;


/* Define driver prototypes.  */

VOID    _nx_nvnet_network_driver(NX_IP_DRIVER *driver_req_ptr);
void    _nx_nvnet_network_driver_output(NX_PACKET *packet_ptr, UINT interface_instance_id);
void    _nx_nvnet_network_driver_receive(NX_IP *ip_ptr, NX_PACKET *packet_ptr, UINT interface_instance_id);
...
typedef struct _nx_nvnet_network_driver_instance_type
{
    UINT          nx_nvnet_network_driver_in_use;

    UINT          nx_nvnet_network_driver_id;

    NX_INTERFACE *nx_nvnet_driver_interface_ptr;

    NX_IP        *nx_nvnet_driver_ip_ptr;

    MAC_ADDRESS   nx_nvnet_driver_mac_address;

    MAC_ADDRESS   nx_nvnet_driver_mcast_address[NX_RAM_DRIVER_MAX_MCAST_ADDRESSES];
} _nx_nvnet_network_driver_instance_type;
...
static _nx_nvnet_network_driver_instance_type nx_nvnet_driver[NX_MAX_RAM_INTERFACES];
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_nvnet_network_driver                              PORTABLE C      */
/*                                                           6.4.0        */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*  CALLS                                                                 */
/*                                                                        */
/*    _nx_nvnet_network_driver_output         Send physical packet out      */
/*                                                                        */
/*  CALLED BY                                                             */
/*                                                                        */
/*    NetX IP processing                                                  */
...
ULONG        *ethernet_frame_ptr;
    ...
    if (driver_req_ptr -> nx_ip_driver_command != NX_LINK_INTERFACE_ATTACH)
    {
        ...
        {
            if (nx_nvnet_driver[i].nx_nvnet_network_driver_in_use == 0)
            {
                continue;
            }

            if (nx_nvnet_driver[i].nx_nvnet_driver_ip_ptr != ip_ptr)
            {
                continue;
            }

            if (nx_nvnet_driver[i].nx_nvnet_driver_interface_ptr == interface_ptr)
            {
                break;
            }
        }
    ...
    case NX_LINK_INTERFACE_ATTACH:
    {
        ...
        {
            /* Set the IN USE flag.*/
            nx_nvnet_driver[i].nx_nvnet_network_driver_in_use  = 1;

            nx_nvnet_driver[i].nx_nvnet_network_driver_id = i;

            /* Record the interface attached to the IP instance. */
            nx_nvnet_driver[i].nx_nvnet_driver_interface_ptr = interface_ptr;

            /* Record the IP instance. */
            nx_nvnet_driver[i].nx_nvnet_driver_ip_ptr = ip_ptr;

            nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_msw = simulated_address_msw;
            nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_lsw = simulated_address_lsw + i;
        }
        else
        {
            driver_req_ptr -> nx_ip_driver_status =  NX_INVALID_INTERFACE;
    ...
    case NX_LINK_INTERFACE_DETACH:
    {
        ...
        memset(&(nx_nvnet_driver[i]), 0, sizeof(_nx_nvnet_network_driver_instance_type));
    ...
    case NX_LINK_INITIALIZE:
    {
        ...
        nx_ip_interface_physical_address_set(ip_ptr, interface_index,
                                             nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_msw,
                                             nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_lsw, NX_FALSE);
    ...
    case NX_LINK_RARP_SEND:
    {
        ...
        *(ethernet_frame_ptr + 3) =  (interface_ptr -> nx_interface_physical_address_lsw << 16) | ether_type;
        ...
        _nx_nvnet_network_driver_output(packet_ptr, i);
        break;
    }
    ...
    case NX_LINK_RAW_PACKET_SEND:
    {
        ...
        _nx_nvnet_network_driver_output(driver_req_ptr -> nx_ip_driver_packet, i);
        break;
    }
#endif /* NX_ENABLE_VLAN */
    ...
    UINT mcast_index;
        ...
        {
            if (nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_msw == 0 &&
                nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_lsw == 0)
            {
                nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_msw = driver_req_ptr -> nx_ip_driver_physical_address_msw;
                nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_lsw = driver_req_ptr -> nx_ip_driver_physical_address_lsw;
                break;
            }
        }
    ...
    UINT mcast_index;
        ...
        for (mcast_index = 0; mcast_index < NX_RAM_DRIVER_MAX_MCAST_ADDRESSES; mcast_index++)
        {
            if (nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_msw == driver_req_ptr -> nx_ip_driver_physical_address_msw &&
                nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_lsw == driver_req_ptr -> nx_ip_driver_physical_address_lsw)
            {
                nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_msw = 0;
                nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_lsw = 0;
                break;
            }
        }
    ...
    case NX_LINK_SET_PHYSICAL_ADDRESS:
    {
        ...
        if (i < NX_MAX_RAM_INTERFACES)
        {
            ...
            nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_msw = driver_req_ptr -> nx_ip_driver_physical_address_msw;
            nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_lsw = driver_req_ptr -> nx_ip_driver_physical_address_lsw;
        }
        else
        {
            driver_req_ptr -> nx_ip_driver_status =  NX_INVALID_INTERFACE;
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_nvnet_network_driver_output                       PORTABLE C      */
/*                                                           6.4.0        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Yuxin Zhou, Microsoft Corporation                                   */
...
/*    _nx_nvnet_network_driver_receive        RAM driver receive processing */
/*                                                                        */
/*  CALLED BY                                                             */
/*                                                                        */
/*    NetX IP processing                                                  */
...
UINT   j;
    ...
    for (i = 0; i < NX_MAX_RAM_INTERFACES; i++)
    {
        ...
        next_ip = nx_nvnet_driver[i].nx_nvnet_driver_ip_ptr;
        ...
        if (((destination_address_msw == ((ULONG)0x0000FFFF)) && (destination_address_lsw == ((ULONG)0xFFFFFFFF))) ||   /* Broadcast match */
            ((destination_address_msw == nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_msw) &&
             (destination_address_lsw == nx_nvnet_driver[i].nx_nvnet_driver_mac_address.nx_mac_address_lsw)) ||
            ...
            ((destination_address_msw == 0) && (destination_address_lsw == 0)))
        {
            ...
            if (nx_packet_copy(packet_ptr, &packet_copy, next_ip -> nx_ip_default_packet_pool, NX_NO_WAIT))
            {
                ...
                nx_link_packet_transmitted(nx_nvnet_driver[interface_instance_id].nx_nvnet_driver_ip_ptr,
                                           nx_nvnet_driver[interface_instance_id].nx_nvnet_driver_interface_ptr -> nx_interface_index,
                                           packet_ptr, NX_NULL);
#else
            ...
            _nx_nvnet_network_driver_receive(next_ip, packet_copy, i);
        }
        else
        {
            for (mcast_index = 0; mcast_index < NX_RAM_DRIVER_MAX_MCAST_ADDRESSES; mcast_index++)
            {
                ...
                if (destination_address_msw == nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_msw &&
                    destination_address_lsw == nx_nvnet_driver[i].nx_nvnet_driver_mcast_address[mcast_index].nx_mac_address_lsw)
                {
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_nvnet_network_driver_receive                      PORTABLE C      */
/*                                                           6.4.0        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Yuxin Zhou, Microsoft Corporation                                   */
...
UINT packet_type;
    ...
    /* Setup interface pointer.  */
    packet_ptr -> nx_packet_address.nx_packet_interface_ptr = nx_nvnet_driver[interface_instance_id].nx_nvnet_driver_interface_ptr;
